...
export default class Map extends Component {
  ...
  componentDidMount() {
    ...
    const currentPath = pathToRegexp('/radar/:name').exec(location.pathname);
    if (currentPath && currentPath[1] === 'map') {
      localStorage.setItem('status', 'map');
    } else {
      localStorage.setItem('status', 'cabinet');
    }
    this.initMap();
  ...
  goDevice = data => () => {
    if (localStorage.getItem('status') === 'cabinet') {
      this.props.history.push(`/radar/${data.deviceId}/status`);
    } else {
      this.props.history.push(`/radar/${data.deviceId}/realtime`);
    }
  }
